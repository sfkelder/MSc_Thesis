...
def output_prediction(inputs, df, model_type='esp'):
    ...
    if model_type == 'esp':
        ...
        model = load_model('../../models/DeepHF_MSc_Thesis_model_twelfe.h5')
    Efficiency = model.predict( inputs )
    df['gRNA_Seq'] = df['21mer'].apply( lambda x: x[:-1] )
    df['Efficiency'] = np.clip( Efficiency, 0, 1 )
    r = model.predict([np.zeros((1, 22)), np.zeros((1,101))])
    #r = model.predict([np.zeros((1, 22)), np.zeros((1,11)), np.zeros((1,101))])
    df = df.drop( ['21mer'], axis=1 )
    df.reset_index( inplace=True )
