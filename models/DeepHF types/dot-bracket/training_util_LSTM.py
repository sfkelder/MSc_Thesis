...
def lstm_model(model_type='esp', batch_size=90, epochs=50, initializer='0',em_dim=44,em_drop=0.2,
    ...
    #new folding bilstm layer
    sequence_input_folding = Input(name = 'seq_input_folding', shape = (101,))

    embedding_layer_folding = Embedding(7,em_dim,input_length=101)
    embedded_folding = embedding_layer(sequence_input_folding)
    ...
    lstm_folding = LSTM(rnn_units, dropout=rnn_drop, 
                kernel_regularizer='l2',recurrent_regularizer='l2',
                recurrent_dropout=rnn_rec_drop, return_sequences=True)
    y = Bidirectional(lstm_folding)(y)
    y = Flatten()(y)
    ...
    #model = Model(inputs=[sequence_input, biological_input, sequence_input_folding], outputs=[mix_output])
    model = Model(inputs=[sequence_input, sequence_input_folding], outputs=[mix_output])
