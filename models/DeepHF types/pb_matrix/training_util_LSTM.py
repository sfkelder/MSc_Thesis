...
def lstm_model(model_type='esp', batch_size=90, epochs=50, initializer='0',em_dim=44,em_drop=0.2,
    ...
    #embedding_layer_folding = Embedding(101,em_dim,input_length=X_train_folding.shape[1])
    #embedded_folding = embedding_layer_folding(folding_input)
    #embedded_folding = SpatialDropout1D(em_drop)(embedded_folding)
    #y = embedded_folding 

    #RNN folding
    lstm_folding = LSTM(rnn_units, dropout=rnn_drop, 
                kernel_regularizer='l2',recurrent_regularizer='l2',
                recurrent_dropout=rnn_rec_drop, return_sequences=True)
    y = Bidirectional(lstm_folding)(folding_input)
    y = Flatten()(y)


    #x = keras.layers.concatenate([x, biological_input, y])
