...
def read_data_files(fn, crisproff, folding):
    d = OrderedDict()
    ...
    with open(folding, 'r') as f:
        ...
        for i,line in enumerate(lines):
            ...
            import ast
            matrix = ast.literal_eval(split[1])
            # matrix = split[1]

            if not len(matrix) == 100:
                  print(i, len(matrix))
                  continue
            d[split[0]].insert(2, matrix)
    return d
...
def onehot_folding(x):
    ...
    for y in list(x):
        if y in ".":  z.append(0)
            ...
            print("Non-.() character " + data[l], file=sys.stderr)
            raise Exception
    return z
...
def preprocess_seq(data):
    ...
    return (seqs, DATA_X, DATA_G, DATA_F)
...
folding = sys.argv[4]
MODELD=sys.argv[5:] #paths to model(s)


d = read_data_files(fasta, crisproff, folding)
(s, x, g, f) = preprocess_seq(d)
tinput = list()
tinput.append(x)
# tinput.append(g)
tinput.append(f)
