...
VAL_F = sys.argv[7] # get value to train from this column 
N_VAL = int(sys.argv[8]) # number of validation set
TEST_N = int(sys.argv[9]) # skip this test set
BATCH_SIZE=int(sys.argv[10]) # batch test 
SEED = int(sys.argv[11]) # common random seed (0 equals randomly set)
TYPE = sys.argv[12] # common random seed (0 equals randomly set)
TRAIN_FP = sys.argv[13:] # path to train sets
...
def read_seq_files(fns, seq_c0, val_c0, val_g0, val_f0, n_val, test_n, test=False):
    d = OrderedDict()
    for j, fn in enumerate(fns):
        ...
        with open(fn, 'rt') as f:
            ...
            for i,l in enumerate(lines):
                ...
                if not len(f) == 100:
                    print('"%f" is not a string (dbn) of length %d in line %d' % (f, 100, i+1))
                    continue
...
def onehot_folding(x):
    z = list()
    for y in list(x):
        if y in ".":  z.append(0)
        elif y in "(": z.append(1)
        elif y in ")": z.append(2)
        else:
            print("Non-.() character " + data[l])
            raise Exception
    return z

def set_data_folding(DX, s):
    for j,x in enumerate(onehot_folding(s)):
        DX[j][x] = 1

def set_data_folding_two(DX, s):
    l = []
    n = 2
    for y in np.arange(len(s)):
      if n >= len(s) - 1:
        continue
       
      for x in np.arange(n, len(s)):
        l.append(float(s[y][x]))

      n += 1     
    DX = l 

def set_data_folding_one(DX, s):
    for y in np.arange(len(s)):
      for x in np.arange(len(s)):
        DX[y][x] = float(s[y][x])
...
def preprocess_seq(data):
    ...
    DATA_F = np.zeros((len(dkeys), 4900, ), dtype=np.float32) 
    ...
    for l, s in enumerate(dkeys):
        ...
        set_data(DATA_X[l], s)
        DATA_G[l] = d[0]
        # set_data_folding_one(DATA_F[l], d[2])
        # DATA_F[l] = [x for xs in d[2] for x in xs]
        set_data_folding_two(DATA_F[l], d[2])
        DATA_Y[l] = d[1]

    return (list(dkeys), DATA_X, DATA_G, DATA_F, DATA_Y)


import tensorflow as tf
from tensorflow.keras import datasets, layers, models, callbacks, Model, optimizers, Input, utils
from tensorflow.keras.layers import LSTM, Conv1D, Dropout, AveragePooling1D, Flatten, Dense, concatenate, SpatialDropout1D
...
#folding
if TYPE.find('F') > -1:
    input_f = Input(shape=(4900,), name="input_folding")
...
#first convolution layer folding
if TYPE.find('F') > -1:
    # conv1_out_f = LSTM(20)(input_f)
    # conv1_out_f = LSTM(50, 2, activation='relu', input_shape=(100,3,), name="conv_3_folding")(input_f)
    # conv1_dropout_out_f = Dropout(0.3, name="drop_3_folding")(conv1_out_f)
    # conv1_pool_out_f = AveragePooling1D(2, padding='SAME', name="pool_3_folding")(conv1_dropout_out_f)
    # conv1_flatten_out_f = Flatten(name="flatten_3_folding")(conv1_out_f)
    for_dense_f.append(input_f)

# #second convolution layer
#     conv2_out_f = Conv1D(30, 4, activation='relu', input_shape=(100,3,), name="conv_5_folding")(input_f)
#     conv2_dropout_out_f = Dropout(0.3, name="drop_5_folding")(conv2_out_f)
#     conv2_pool_out_f = AveragePooling1D(2, padding='SAME', name="pool_5_folding")(conv2_dropout_out_f)
#     conv2_flatten_out_f = Flatten(name="flatten_5_folding")(conv2_pool_out_f)
#     for_dense_f.append(conv2_flatten_out_f)

# #third convolution layer
#     conv3_out_f = Conv1D(15, 5, activation='relu', input_shape=(100,3,), name="conv_7_folding")(input_f)
#     conv3_dropout_out_f = Dropout(0.3, name="drop_7_folding")(conv3_out_f)
#     conv3_pool_out_f = AveragePooling1D(2, padding='SAME', name="pool_7_folding")(conv3_dropout_out_f)
#     conv3_flatten_out_f = Flatten(name="flatten_7_folding")(conv3_pool_out_f)
#     for_dense_f.append(conv3_flatten_out_f)

#concatenation of conv layers and folding layer
    if len(for_dense_f) == 1:
        concat_out_f = for_dense_f[0]
    else:
        concat_out_f = concatenate(for_dense_f)

#first dense (fully connected) layer
    dense0_out_f = Dense(80, activation='relu', name="dense_0_folding")(concat_out_f)
    dense0_dropout_out_f = Dropout(0.3, name="drop_d0_folding")(dense0_out_f)
    for_dense1.append(dense0_dropout_out_f)
...
d = read_seq_files(TRAIN_FP, SEQ_C, VAL_C, VAL_G, VAL_F, N_VAL, TEST_N, test=False)
(s, x, g, f, y) = preprocess_seq(d)

print('reading validation data')
db = read_seq_files(TRAIN_FP, SEQ_C, VAL_C, VAL_G, VAL_F, N_VAL, TEST_N, test=True)
(sv, xv, gv, fv, yv) = preprocess_seq(db)
